...
(function() {
  ...
  function createEdges(imageData, options) {
    ...
    var height = imageData.height;
    var rgbData = imageData.data;
    var edgeSize = 4 * width * height - 3 * width - 3 * height + 2;
    var edges = {
  ...
  function sortEdgesByWeights(edges) {
    ...
    var temporaryB = new Uint32Array(b);
    var temporaryW = new Float32Array(w);
    for (var i = 0; i < order.length; ++i) {
  ...
  function segmentGraph(imageData, options) {
    ...
    var edges = createEdges(imageData, options);
  ...
  function createIndexMap(universe, imageData, options) {
    var width = imageData.width;
    var height = imageData.height;
    var indexMap = new Int32Array(width * height);
    var nodeIds = [];
    var lastId = 0;
    for (var i = 0; i < height; ++i) {
      for (var j = 0; j < width; ++j) {
        var component = findNode(universe, i * width + j);
        var index = nodeIds[component];
        if (index === undefined) {
          index = lastId;
  ...
  function computeSegmentation(imageData, options) {
    ...
    var indexMap = createIndexMap(universe, imageData, options);
    if (options.callback) {
